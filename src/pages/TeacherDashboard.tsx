          ...
          >
            <div className="slot-modal-header">
              <div className="slot-modal-title-row">
                ...
                <h4>Choose Session Slot</h4>
              </div>
              <span className="slot-modal-day-badge">{manualDay}</span>
            </div>
            {daySlots.length === 0 ? (
                          ...
                          {slot.entries.length > 1 ? (
                            <div className="slot-lines">
                              {slot.entries.map((entry) => (
                                <div className="slot-entry-row" key={`${entry.batch}-${entry.subject}`}>
                                  <span className="slot-entry-batch">{entry.batch ?? 'All'}</span>
                                  <span className="slot-entry-subject">{entry.subject}</span>
                                  {entry.room && <span className="slot-entry-room">Room {entry.room}</span>}
                                </div>
                              ))}
                            </div>
                          ) : (
                            <div className="slot-single-entry">
                              <span className="slot-entry-subject">{slot.entries[0].subject}</span>
                              {slot.entries[0].room && <span className="slot-entry-room">Room {slot.entries[0].room}</span>}
                            </div>
                          )}
                        </div>
                      </div>
